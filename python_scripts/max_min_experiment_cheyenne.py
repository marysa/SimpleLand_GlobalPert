...
# roughness:
ds_low['hc'] = ds_cam['global_a2_cv2_hc0.01_rs100_cheyenne']
ds_med['hc'] = ds_cam['global_a2_cv2_hc0.1_rs100_cheyenne']
ds_high['hc'] = ds_cam['global_a2_cv2_hc2.0_rs100_cheyenne']

dsl_low['hc'] = ds_clm['global_a2_cv2_hc0.01_rs100_cheyenne']
dsl_med['hc'] = ds_clm['global_a2_cv2_hc0.1_rs100_cheyenne']
dsl_high['hc'] = ds_clm['global_a2_cv2_hc2.0_rs100_cheyenne']
...
#%%
# Do deltas of highest and lowest experiment - map total dT between them
"""
    Plot the experiment with the maximum (and minimum) temperature.
    Ie, albedo should be "1" everywhere. If hc is parabolic, it should be 2, while
    if it is more linear, it should be 1 or 3. Plot minimum, aslo.
...
for prop in sfc_props: 
    ...
    mapdata_full = np.array(ds_high[prop][myvar])-np.array(ds_low[prop][myvar])
    
    mapdata_ann = np.mean(mapdata_full,0)
    mapdata_djf = np.mean(mapdata_full[[11,0,1],:,:],0)
    mapdata_mam = np.mean(mapdata_full[[2,3,4],:,:],0)
    mapdata_jja = np.mean(mapdata_full[[5,6,7],:,:],0)
    mapdata_son = np.mean(mapdata_full[[8,9,10],:,:],0)
    ...
    ax0 = plt.gca()
    #plt.sca(ax0)
    ttl = 'Annual delta '+myvar+', exp high - exp low; ANN; '+prop
    
    
    #clim_diff = [-.01,.01]
    mapdata = mapdata_ann
    mml_map(LN,LT,mapdata,ds0,myvar,'moll',title=ttl,clim=clim,colmap=cmap_diff, cb_ttl='units: '+units )   #plt.cm.BuPu_r
    ax=ax0
    for item in ([ax.title, ax.xaxis.label, ax.yaxis.label] + ax.get_xticklabels() + ax.get_yticklabels()):
        item.set_fontsize(12)
    
    filename = 'high_m_low_'+prop+'_'+mask_name+'_'+'ann'
    fig_name = figpath+'/sensitivity/deltas/'+filename+'.png'
    fig.savefig(fig_name,dpi=1200,transparent=True,facecolor='w',
                edgecolor='w',orientation='portrait',bbox_inches='tight', 
                pad_inches=0.1,frameon=None)
    ...
    ax0 = plt.gca()
    #plt.sca(ax0)
    ttl = 'Annual delta '+myvar+', exp high - exp low; DJF; '+prop
    ...
    filename = 'high_m_low_'+prop+'_'+mask_name+'_'+'djf'
    fig_name = figpath+'/sensitivity/deltas/'+filename+'.png'
    fig.savefig(fig_name,dpi=1200,transparent=True,facecolor='w',
                edgecolor='w',orientation='portrait',bbox_inches='tight', 
                pad_inches=0.1,frameon=None)
    ...
    ax0 = plt.gca()
    #plt.sca(ax0)
    ttl = 'Annual delta '+myvar+', exp high - exp low; JJA; '+prop
    ...
    filename = 'high_m_low_'+prop+'_'+mask_name+'_'+'jja'
    fig_name = figpath+'/sensitivity/deltas/'+filename+'.png'
    fig.savefig(fig_name,dpi=1200,transparent=True,facecolor='w',
                edgecolor='w',orientation='portrait',bbox_inches='tight', 
                pad_inches=0.1,frameon=None)
    ...
    ax0 = plt.gca()
    #plt.sca(ax0)
    ttl = 'Annual delta '+myvar+', exp high - exp low; MAM; '+prop
    ...
    filename = 'high_m_low_'+prop+'_'+mask_name+'_'+'mam'
    fig_name = figpath+'/sensitivity/deltas/'+filename+'.png'
    fig.savefig(fig_name,dpi=1200,transparent=True,facecolor='w',
                edgecolor='w',orientation='portrait',bbox_inches='tight', 
                pad_inches=0.1,frameon=None)
    ...
    ax0 = plt.gca()
    #plt.sca(ax0)
    ttl = 'Annual delta '+myvar+', exp high - exp low; SON'
    ...
    filename = 'high_m_low_'+prop+'_'+mask_name+'_'+'son'
    fig_name = figpath+'/sensitivity/deltas/'+filename+'.png'
    fig.savefig(fig_name,dpi=1200,transparent=True,facecolor='w',
                edgecolor='w',orientation='portrait',bbox_inches='tight', 
                pad_inches=0.1,frameon=None)
