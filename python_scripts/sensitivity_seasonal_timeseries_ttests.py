...
# -*- coding: utf-8 -*-
"""
Created on Fri Oct 13 13:06:57 2017

@author: mlague


Sensitivity plots carrying information for t-tests on map plots! ie, must
do it for each time slice (e.g. year, or month... lets load it for each month)
...
atm_vars = ['TREFHT','TS','SHFLX','LHFLX']
...
for run in sims:
    ...
    for var in atm_vars:
        cam_files[run][var] = ext_dir + run + '/TimeSeries/' + run + '.cam.h0.ts.20-50.' + var + '.nc'
    
        ds_cam[run][var] = xr.open_dataset(cam_files[run][var])
    
    for var in lnd_vars:
        clm_files[run][var] = ext_dir + run + '/TimeSeries/' + run + '.clm2.h0.ts.20-50.' + var + '.nc'
    
        ds_clm[run][var] = xr.open_dataset(clm_files[run][var])
...
for prop in ['rs']:
    ...
    for sea in ['ANN']:
        ...
        AUS_ann_fsnsc = (np.nanmean(ctrl_precip,0)[ind_bot:ind_top,ind_left:ind_right] *
                             no_glc[ind_bot:ind_top,ind_left:ind_right] )
        AUS_yvar = AUS_ann_fsnsc.flatten()*ms2mmyr
