...
def get_online(alb=None, rs=None, hc=None):
    #-----------------------------
    # Inputs:
    #
    ...
    ext_dir = '/home/disk/eos18/mlague/simple_land/output/future_m_hist_albedo/'
    
    
    sims = {}
    sims['alb'] = [ '1850_historical_albedo',
                    '10x_rcp85_m_hist_albedo']
    #sims['rs'] =  [ ]
    #sims['hc'] =  [ ]
 
    #print(sims)
    
    run0 = '1850_historical_albedo'
    print(ext_dir + run0 + '/means/' + run0 + '.clm2.h0.20-50_year_avg.nc')
    ...
    if alb:
        ...
        for run in sims[prop]:
            for var in varlist_atm:
                ...
                ds_cam_ts[prop][run][var] = xr.open_dataset(ext_dir + run + '/TimeSeries/' + run + '.cam.h0.20-50.' + var + '.nc')
            for var in varlist_lnd:
                ds_clm_ts[prop][run] = {}
                ds_clm_ts[prop][run][var] = xr.open_dataset(ext_dir + run + '/TimeSeries/' + run + '.clm2.h0.20-50.' + var + '.nc')
...
def get_offline(alb=None, rs=None, hc=None):
    ...
    sims = {}
    sims['alb'] = [ '1850_historical_albedo_offline',
                    '10x_rcp85_m_hist_albedo_offline_snowmask']
    #sims['rs'] =  [ ]
    #sims['rs'] =  [   ]
    #sims['hc'] =  [   ]
...
def get_offline_ts(alb=None, rs=None, hc=None,varlist_lnd=None):
    ...
    # rs : if rs==True, load evaporative resistance runs
    # hc : if hc==True, load roughenss runs
    # varlist = list of time series variables to load (offline sims, must come from clm h0)
    #
    # Returns:
    #
    # ds_cam: dictionary of xarray netcdfs for cam output as [prop][run]
