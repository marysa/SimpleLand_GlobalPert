...
# Populate a list of paired strings of prop-sea to feed to the parallelizer
num_its = np.size(sfc_props)*np.size(seasons)
iter_list = range(num_its)
par_prop=[]
par_sea=[]
...
for prop in sfc_props:
    ...
    for sea in seasons:
        ...
        par_prop.append(prop)
        par_sea.append(sea)

num_cores = multiprocessing.cpu_count()

slope, intercept, r_value, p_value = Parallel(n_jobs=num_cores)(delayed(calculate_slopes)(par_prop(i),par_sea(i)) for i in iter_list )
        
#%%    
        

def calculate_slopes(prop,sea):
